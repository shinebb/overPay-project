...
public class MemberDAO extends JDBConnect{
	...
	public MemberDTO getMemberDTO(String uid, String upass) {
		...
		try {
			...
			if(rs.next()) {
				...
				dto.setBirthday(rs.getString(4));  //4번째변수..
	...
	public int insertUser(MemberDTO dto) {
		...
		try {
			...
			String query = "INSERT INTO O_MEMBER ( "
					+ " id, pass, name, birthday, gender, email, regidate) "
					+ " VALUES ( "
					+ " ?, ?, ?, ?, ?, ?, SYSDATE)";
	...
	//아이디 찾기
	public String findId(String user_name, String email) {
		...
		try {
			...
			psmt.setString(2, email); //쿼리문의 두 번째 인파리터에 값 설정
			rs = psmt.executeQuery(); //쿼리문 실행
	...
	//비밀번호 찾기
	public String findPw(String user_id, String user_name, String email) {
		...
		try {
			...
			psmt.setString(3, email); //쿼리문의 세 번째 인파리터에 값 설정
			rs = psmt.executeQuery(); //쿼리문 실행
